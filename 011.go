...
func findProduct(x, y, dx, dy int) {
	product := 1
	for i:=0; i<numCount; i++ {
		nx := x + (i * dx); ny := y + (i * dy)
		if nx < 0 || ny < 0 || nx >= len(data[0]) || ny >= len(data) { return }
		product *= data[ny][nx]
	}
...
func main() {
	for _,line := range strings.Split(input, "\n") {
		...
		for _,n := range strings.Split(line, " ") {
			...
			data[len(data)-1] = append(data[len(data)-1], num)
	...
	for j,list := range data {
		for i,_ := range list {
			for _,o := range offsets {
				findProduct(i, j, o[0],o[1])
			}
